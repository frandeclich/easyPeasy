...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			res.write(`\n`)
			res.write('Películas: \n')
			function nombrarOrdenado (){for(let i=0;i<titulosOrdenados.length;i++){
				res.write(`${[i+1]}- ${titulosOrdenados[i]}`)
				res.write(`\n`)
			}}
			nombrarOrdenado()
			res.write('\n') 
		...
		case '/en-cartelera':
			...
			res.write('\n')
			res.write(`Total de películas en cartelera: ${movies.length}\n`)			
			function nombrarReseñado (){for(let i=0;i<movies.length;i++){
				res.write(`${[i+1]}- ${movies[i].title}\n Reseña: ${movies[i].overview}\n`)
				res.write(`\n`)
			}}
			nombrarReseñado()
			res.end();			
		...
		case '/mas-votadas':
			...
			res.write('\nTotal de películas: \n')
			let pelisVotadas=movies.filter(function(movie){
				return movie.vote_average>=7
			})
			function nombrarLasBuenas(){
				for(let i=0;i<pelisVotadas.length;i++){
				res.write(`${[i+1]}- ${pelisVotadas[i].title}`)
				res.write(`\n`)
			}}
			nombrarLasBuenas()			
			let suma2=pelisVotadas.map(function(peli){
				return peli.vote_average})
			let suma=suma2.reduce(function(acumulador,peli){
				return acumulador+peli})
			let promedio=suma/pelisVotadas.length
			let promedioRedondeado=promedio.toFixed(2)			
			res.write(`\nPromedio: ${promedioRedondeado}`)
			function nombrarPelisVotadas(){
				for(let i=0;i<theaters.length;i++){
					res.write(`\n.${i+1}\n i- Nombre: ${theaters[i].name}\n ii- Dirección: ${theaters[i].address}\n iii- Descripción: ${theaters[i].description}\n`)
				}
			}
			nombrarPelisVotadas()
			res.end();
		...
		case '/sucursales':
			...
			res.write(`Cantidad de salas disponibles: ${theaters.length}\n`)
			function nombrarSalas(){
				for(let i=0;i<theaters.length;i++){
					res.write(`\n.${i+1}\ni- Nombre: ${theaters[i].name}\nii- Dirección: ${theaters[i].address}\niii- Descripción: ${theaters[i].description}\n`)
				}
			}
		...
		case '/contacto':
			...
			res.write(`\n`)
			res.write('¿Tenés algo para contarnos? Nos encanta escuchar a nuestros clientes. Si deseas contactarnos podés escribirnos al siguiente email: dhmovies@digitalhouse.com o en las redes sociales. Envianos tu consulta,sugerencia o reclamo y será respondido a la brevedad posible. Recordá que también podés consultar la sección de Preguntas Frecuentes para obtener respuestas inmediatas a los problemas más comunes.')
			res.end();
			break;
		case '/preguntas-frecuentes':
			res.write(`Preguntas Frecuentes.\n`)
			res.write(`Cantidad de preguntas: ${faqs.length}\n`)
			function nombrarPreguntasYRespuestas(){
				for(let i=0;i<faqs.length;i++){
					res.write(`\n.${i}\ni. Pregunta: ${faqs[i].faq_title}\nii. Respuesta: ${faqs[i].faq_answer}\n`)
				}
			}
			nombrarPreguntasYRespuestas()
			res.end(); 
			break;
		default:
...
function nombrarLasBuenas(){for(let i=0;i<pelisVotadas.length;i++){
	console.log([i+1]+'- '+pelisVotadas[i].title)
}}
nombrarLasBuenas() */
...
let suma3=pelisVotadas.reduce(function(acumulador,peli){
	return acumulador+Number.(peli.vote_average)})
let promedio=suma3/pelisVotadas.length
